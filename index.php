...
.empresa-card:hover { border-color:var(--blue); transform:translateY(-1px); }
.empresa-card.selected { border-color:var(--blue); background:rgba(59,130,246,.06); }
.empresa-card::before { content:''; position:absolute; top:0; left:0; right:0; height:2px; background:linear-gradient(90deg,var(--blue),var(--purple)); }
.empresa-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(240px,1fr)); gap:14px; margin-bottom:20px; }
.empresa-nome { font-weight:700; font-size:.95rem; margin-bottom:4px; }
.empresa-cnpj { font-size:.72rem; color:var(--text3); font-family:'JetBrains Mono',monospace; margin-bottom:10px; }
.empresa-stats { display:flex; gap:12px; }
.empresa-stat { font-size:.72rem; color:var(--text2); }
.empresa-stat strong { color:var(--text); }
...
.cliente-row:hover td { background:rgba(59,130,246,.05) !important; }

.status-pago     { color:var(--green); font-weight:600; font-size:.78rem; }
.status-pendente { color:var(--yellow); font-weight:600; font-size:.78rem; }
.status-cancelado{ color:var(--text3); font-size:.78rem; }
...
.emp-tab {
  ...
  border-radius:var(--radius) var(--radius) 0 0; color:var(--text3);
  border:1px solid transparent; border-bottom:none;
  transition:all var(--transition);
}
.emp-tab:hover { color:var(--text); background:var(--card2); }
.emp-tab.active { color:var(--blue); background:var(--card); border-color:var(--border); border-bottom-color:var(--card); margin-bottom:-1px; }

.venc-chip {
  display:inline-block; padding:2px 8px; border-radius:99px; font-size:.7rem; font-weight:600;
}
.venc-chip.atrasado { background:rgba(239,68,68,.15); color:var(--red); }
.venc-chip.hoje     { background:rgba(245,158,11,.15); color:var(--yellow); }
.venc-chip.proximo  { background:rgba(59,130,246,.15); color:var(--blue); }
.venc-chip.ok       { background:rgba(100,116,139,.12); color:var(--text3); }
</style>
</head>
<body>
<div class="app-wrap">
...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <div class="nav-label" style="margin-top:8px">Minhas Empresas</div>
    <div class="nav-item" onclick="showView('empresas')">
      <?= icon('building') ?> Empresas
    </div>
    <div class="nav-item" onclick="showView('emp-servicos')">
      <?= icon('monitor') ?> Serviços TI
    </div>
  </nav>
  ...
  <div class="content">
    ...
    <div id="view-empresas" class="view">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
        <div>
          <div style="font-size:.8rem;color:var(--text3)" id="empSubtitulo">Selecione uma empresa para gerenciar clientes e serviços</div>
        </div>
      ...
      <!-- Painel interno da empresa selecionada -->
      <div id="empPainel" style="display:none">
        <div class="breadcrumb">
          <a onclick="fecharEmpPainel()">🏢 Empresas</a>
          <span class="breadcrumb-sep">›</span>
          <span id="empNomeAtual">–</span>
        </div>

        <!-- Resumo da empresa -->
        <div class="summary-grid" style="margin-bottom:16px" id="empResumoCards">
          <div class="summary-card blue"><div class="sc-label">Clientes</div><div class="sc-value blue" id="empTotalClientes">0</div></div>
          <div class="summary-card yellow"><div class="sc-label">Serviços Pendentes</div><div class="sc-value yellow" id="empTotalPendentes">0</div></div>
          <div class="summary-card red"><div class="sc-label">Valor Pendente</div><div class="sc-value red" id="empValorPendente">R$ 0,00</div></div>
          <div class="summary-card green"><div class="sc-label">Valor Recebido</div><div class="sc-value green" id="empValorRecebido">R$ 0,00</div></div>
        </div>

        <!-- Tabs -->
        <div class="emp-tabs">
          <div class="emp-tab active" id="tabClientes" onclick="showEmpTab('clientes')">👥 Clientes</div>
          <div class="emp-tab" id="tabServicos" onclick="showEmpTab('servicos')">🖥️ Serviços</div>
        </div>

        <!-- Tab Clientes -->
        <div id="empTabClientes">
          <div style="display:flex;justify-content:flex-end;margin-bottom:12px">
            <button class="btn btn-primary btn-sm" onclick="openClienteModal()">+ Novo Cliente</button>
          </div>
          <div class="panel">
            <div class="table-wrap">
              <table>
                <thead><tr><th>Nome</th><th>E-mail</th><th>Telefone</th><th>Pendentes</th><th>Valor Aberto</th><th>Ações</th></tr></thead>
        ...
        <!-- Tab Serviços da Empresa -->
        <div id="empTabServicos" style="display:none">
          <div style="display:flex;justify-content:flex-end;margin-bottom:12px">
            <button class="btn btn-primary btn-sm" onclick="openServicoModal()">+ Novo Serviço</button>
          </div>
          <div class="panel">
            <div class="table-wrap">
              <table>
                <thead><tr><th>Serviço</th><th>Cliente</th><th>Vencimento</th><th>Situação</th><th>Valor</th><th>Status</th><th>Ações</th></tr></thead>
                <tbody id="tabelaServicosEmp"></tbody>
              </table>
            </div>
          </div>
        </div>
    ...
    <!-- ════════════════ SERVIÇOS TI (visão geral) ════════════════ -->
    <div id="view-emp-servicos" class="view">
      <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:16px;align-items:center">
        <select id="filtroEmpServico" onchange="loadServicosGeral()" style="min-width:180px">
          <option value="">Todas as empresas</option>
        </select>
        <select id="filtroStatusServico" onchange="loadServicosGeral()">
          <option value="">Todos os status</option>
          <option value="pendente">Pendente</option>
          <option value="pago">Pago</option>
        </select>
      ...
      <div class="panel">
        <div class="panel-header">
          <span class="panel-title" id="servicosGeralTitle">Serviços de TI</span>
          <button class="btn btn-primary btn-sm" onclick="openServicoModal(true)">+ Novo Serviço</button>
        </div>
        <div class="table-wrap">
          <table>
            <thead><tr><th>Serviço</th><th>Empresa</th><th>Cliente</th><th>Vencimento</th><th>Situação</th><th>Valor</th><th>Status</th><th>Ações</th></tr></thead>
            <tbody id="tabelaServicosGeral"></tbody>
          </table>
        </div>
      </div>
    </div>
...
<div class="modal-overlay" id="modalEmpresa">
  <div class="modal">
    <div class="modal-header">
      <span class="modal-title" id="empresaModalTitle">Nova Empresa</span>
      <button class="close-btn" onclick="closeModal('modalEmpresa')">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
      </button>
    </div>
    <div class="modal-body">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input type="text" class="form-control" id="empresaNome" placeholder="Razão social ou nome fantasia">
        </div>
        <div class="form-group">
          <label class="form-label">CNPJ</label>
          <input type="text" class="form-control" id="empresaCnpj" placeholder="00.000.000/0001-00">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label class="form-label">E-mail</label>
          <input type="text" class="form-control" id="empresaEmail" placeholder="contato@empresa.com.br">
        </div>
        <div class="form-group">
          <label class="form-label">Telefone</label>
          <input type="text" class="form-control" id="empresaTelefone" placeholder="(11) 99999-9999">
        </div>
      </div>
      <div class="form-group">
        <label class="form-label">Observação</label>
        <textarea class="form-control" id="empresaObs" rows="2" placeholder="Notas sobre a empresa…" style="resize:vertical"></textarea>
      </div>
    </div>
    <div class="modal-footer">
...
<div class="modal-overlay" id="modalCliente">
  <div class="modal">
    <div class="modal-header">
      ...
      <button class="close-btn" onclick="closeModal('modalCliente')">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
      </button>
    </div>
    <div class="modal-body">
      <input type="hidden" id="clienteId">
      <input type="hidden" id="clienteEmpresaId">
      <div class="form-group">
        <label class="form-label">Nome do Cliente *</label>
        <input type="text" class="form-control" id="clienteNome" placeholder="Nome completo ou razão social">
      </div>
      <div class="form-row">
        <div class="form-group">
          <label class="form-label">E-mail</label>
          <input type="text" class="form-control" id="clienteEmail" placeholder="cliente@email.com">
        </div>
        <div class="form-group">
          <label class="form-label">Telefone</label>
      ...
      <div class="form-group">
        <label class="form-label">Observação</label>
        <textarea class="form-control" id="clienteObs" rows="2" placeholder="Notas sobre o cliente…" style="resize:vertical"></textarea>
      </div>
    </div>
    <div class="modal-footer">
...
<div class="modal-overlay" id="modalServico">
  <div class="modal">
    <div class="modal-header">
      <span class="modal-title" id="servicoModalTitle">Novo Serviço</span>
      <button class="close-btn" onclick="closeModal('modalServico')">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
      </button>
    </div>
    <div class="modal-body">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <select class="form-control" id="servicoEmpresaId" onchange="loadClientesNoSelect()"></select>
        </div>
        <div class="form-group">
          <label class="form-label">Cliente *</label>
          <select class="form-control" id="servicoClienteId"></select>
        </div>
      </div>
      <div class="form-group">
        <label class="form-label">Nome do Serviço *</label>
        <input type="text" class="form-control" id="servicoNome" placeholder="Ex: Hospedagem, Manutenção, Desenvolvimento…">
      </div>
      <div class="form-group">
        <label class="form-label">Descrição</label>
        <textarea class="form-control" id="servicoDesc" rows="2" placeholder="Detalhes do serviço…" style="resize:vertical"></textarea>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label class="form-label">Valor *</label>
          <input type="text" class="form-control" id="servicoValor" placeholder="0,00" oninput="maskMoney(this)">
        </div>
        <div class="form-group">
          <label class="form-label">Vencimento *</label>
          <input type="date" class="form-control" id="servicoVenc">
        </div>
      </div>
      <div class="form-group">
        <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:.8rem;color:var(--text2)">
          <input type="checkbox" id="servicoRecorrente"> Serviço recorrente (mensal)
        </label>
      </div>
    </div>
    <div class="modal-footer">
      <button class="btn btn-secondary" onclick="closeModal('modalServico')">Cancelar</button>
...
<!-- ══════════ MODAL PAGAR SERVIÇO ══════════ -->
<div class="modal-overlay" id="modalPagarServico">
  <div class="modal">
    <div class="modal-header">
      <span class="modal-title">Confirmar Recebimento</span>
      <button class="close-btn" onclick="closeModal('modalPagarServico')">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
      </button>
    </div>
    <div class="modal-body">
      <p style="font-size:.875rem;color:var(--text2);margin-bottom:16px" id="pagarServicoDesc">–</p>
      <div class="form-group">
        <label class="form-label">Data do Recebimento</label>
        <input type="date" class="form-control" id="dataPagamentoServico">
      </div>
      <input type="hidden" id="pagarServicoId">
    </div>
    <div class="modal-footer">
      <button class="btn btn-secondary" onclick="closeModal('modalPagarServico')">Cancelar</button>
      <button class="btn btn-primary" onclick="confirmarPagamentoServico()">✅ Confirmar Recebimento</button>
    </div>
  </div>
</div>
...
function showView(name) {
  ...
  document.querySelectorAll('.nav-item').forEach(i => {
    ...
    if (name === 'dashboard'     && txt.includes('dashboard'))   i.classList.add('active');
    if (name === 'transacoes'    && txt.includes('transa'))      i.classList.add('active');
    if (name === 'pendentes'     && txt.includes('pend'))        i.classList.add('active');
    if (name === 'relatorio'     && txt.includes('relat'))       i.classList.add('active');
    if (name === 'categorias'    && txt.includes('categ'))       i.classList.add('active');
    if (name === 'empresas'      && txt.includes('empresa'))     i.classList.add('active');
    if (name === 'emp-servicos'  && txt.includes('servi'))       i.classList.add('active');
  });
  ...
  else if (name === 'empresas')     loadEmpresas();
  else if (name === 'emp-servicos') { loadEmpresas(); loadServicosGeral(); }
}
...
async function loadEmpresas() {
  const data = await api({ action: 'empresas_listar' });
  empState.todasEmpresas = data.empresas || [];
  renderEmpresaGrid(empState.todasEmpresas);
  fillEmpresaSelects(empState.todasEmpresas);
}

function renderEmpresaGrid(empresas) {
  const grid = document.getElementById('empresaGrid');
  if (!empresas.length) {
    grid.innerHTML = `<div class="empty" style="grid-column:1/-1"><p>Nenhuma empresa cadastrada ainda. Clique em "+ Nova Empresa" para começar.</p></div>`;
    return;
  }
  grid.innerHTML = empresas.map(e => `
    <div class="empresa-card ${empState.empresaId == e.id ? 'selected' : ''}" onclick="selecionarEmpresa(${e.id},'${esc(e.nome)}')">
      <div class="empresa-nome">🏢 ${esc(e.nome)}</div>
      <div class="empresa-cnpj">${e.cnpj || 'CNPJ não informado'}</div>
      <div class="empresa-stats">
        <div class="empresa-stat"><strong>${e.total_clientes || 0}</strong> clientes</div>
        <div class="empresa-stat"><strong>${e.servicos_pendentes || 0}</strong> pendentes</div>
        ${parseFloat(e.valor_pendente||0) > 0 ? `<div class="empresa-stat" style="color:var(--red)"><strong>${fmtMoney(e.valor_pendente)}</strong></div>` : ''}
      </div>
      <div style="position:absolute;top:12px;right:12px;display:flex;gap:4px">
        <button class="btn btn-icon btn-secondary btn-sm" onclick="event.stopPropagation();editarEmpresa(${e.id})" title="Editar">✏️</button>
        <button class="btn btn-icon btn-danger btn-sm" onclick="event.stopPropagation();excluirEmpresa(${e.id})" title="Excluir">🗑️</button>
      </div>
    </div>
  `).join('');
}

function fillEmpresaSelects(empresas) {
  ['filtroEmpServico','servicoEmpresaId'].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    const val = el.value;
    el.innerHTML = (id === 'filtroEmpServico' ? '<option value="">Todas as empresas</option>' : '<option value="">Selecione a empresa</option>');
    empresas.forEach(e => { el.innerHTML += `<option value="${e.id}">${esc(e.nome)}</option>`; });
    if (val) el.value = val;
  });
}

async function selecionarEmpresa(id, nome) {
  empState.empresaId   = id;
  empState.empresaNome = nome;
  document.getElementById('empNomeAtual').textContent = nome;
  document.getElementById('empPainel').style.display  = 'block';
  document.getElementById('empSubtitulo').textContent = nome;
  renderEmpresaGrid(empState.todasEmpresas);
  showEmpTab('clientes');
  loadEmpResumo(id);
}

function fecharEmpPainel() {
  empState.empresaId = null;
  document.getElementById('empPainel').style.display = 'none';
  document.getElementById('empSubtitulo').textContent = 'Selecione uma empresa para gerenciar clientes e serviços';
  renderEmpresaGrid(empState.todasEmpresas);
}

function showEmpTab(tab) {
  empState.tabAtual = tab;
  document.getElementById('empTabClientes').style.display = tab === 'clientes' ? 'block' : 'none';
  document.getElementById('empTabServicos').style.display = tab === 'servicos' ? 'block' : 'none';
  document.getElementById('tabClientes').className = 'emp-tab ' + (tab === 'clientes' ? 'active' : '');
  document.getElementById('tabServicos').className = 'emp-tab ' + (tab === 'servicos' ? 'active' : '');
  if (tab === 'clientes') loadClientes();
  else loadServicosEmp();
}

async function loadEmpResumo(empresa_id) {
  const data = await api({ action: 'empresa_resumo', empresa_id });
  const r = data.resumo || {};
  document.getElementById('empTotalPendentes').textContent = r.pendentes || 0;
  document.getElementById('empValorPendente').textContent  = fmtMoney(r.valor_pendente || 0);
  document.getElementById('empValorRecebido').textContent  = fmtMoney(r.valor_recebido || 0);
}

// ── EMPRESAS CRUD ─────────────────────────────
function openEmpresaModal(id = 0) {
...
async function editarEmpresa(id) {
  ...
  document.getElementById('empresaObs').value      = e.observacao || '';
  document.getElementById('modalEmpresa').classList.add('open');
}
...
async function salvarEmpresa() {
  const body = {
    ...
    observacao: document.getElementById('empresaObs').value.trim(),
  };
  if (!body.nome) { toast('Nome da empresa é obrigatório.', 'error'); return; }
  const data = await api({ action: 'empresa_salvar' }, 'POST', body);
  if (data.erro) { toast(data.erro, 'error'); return; }
  toast('Empresa salva!');
...
async function excluirEmpresa(id) {
  if (!confirm('Excluir esta empresa e todos os seus clientes e serviços?')) return;
  const fd = new FormData(); fd.append('id', id);
  const data = await apiPost('empresa_excluir', fd);
  if (data.erro) { toast(data.erro, 'error'); return; }
  toast('Empresa excluída!');
  if (empState.empresaId == id) fecharEmpPainel();
  loadEmpresas();
}

// ── CLIENTES CRUD ─────────────────────────────
async function loadClientes() {
  if (!empState.empresaId) return;
  const data = await api({ action: 'clientes_listar', empresa_id: empState.empresaId });
  const rows = data.clientes || [];
  document.getElementById('empTotalClientes').textContent = rows.length;
  const tb = document.getElementById('tabelaClientes');
  if (!rows.length) { tb.innerHTML = `<tr><td colspan="6"><div class="empty"><p>Nenhum cliente cadastrado ainda.</p></div></td></tr>`; return; }
  tb.innerHTML = rows.map(c => `
    <tr>
      <td style="font-weight:600">👤 ${esc(c.nome)}</td>
      <td style="color:var(--text2);font-size:.8rem">${c.email || '–'}</td>
      <td style="color:var(--text2);font-size:.8rem">${c.telefone || '–'}</td>
      <td>${c.servicos_pendentes > 0 ? `<span class="badge badge-yellow">${c.servicos_pendentes}</span>` : '<span class="badge badge-gray">0</span>'}</td>
      <td><span class="valor-mono ${parseFloat(c.valor_pendente||0)>0?'chip-despesa':''}">${parseFloat(c.valor_pendente||0)>0?fmtMoney(c.valor_pendente):'–'}</span></td>
      <td><div class="actions">
        <button class="btn btn-secondary btn-sm btn-icon" onclick="editarCliente(${c.id})" title="Editar">✏️</button>
        <button class="btn btn-primary btn-sm" onclick="novoServicoParaCliente(${c.id},'${esc(c.nome)}')" title="Novo serviço">+ Serviço</button>
        <button class="btn btn-danger btn-sm btn-icon" onclick="excluirCliente(${c.id})" title="Excluir">🗑️</button>
      </div></td>
    </tr>
  `).join('');
}
...
function openClienteModal() {
  ...
  document.getElementById('clienteEmpresaId').value  = empState.empresaId;
...
async function editarCliente(id) {
  const data = await api({ action: 'cliente_buscar', id });
  ...
  document.getElementById('clienteObs').value       = c.observacao || '';
  document.getElementById('modalCliente').classList.add('open');
}
...
async function salvarCliente() {
  const body = {
    ...
    empresa_id: document.getElementById('clienteEmpresaId').value,
    nome:       document.getElementById('clienteNome').value.trim(),
    email:      document.getElementById('clienteEmail').value.trim(),
    telefone:   document.getElementById('clienteTelefone').value.trim(),
    observacao: document.getElementById('clienteObs').value.trim(),
  };
  if (!body.nome) { toast('Nome do cliente é obrigatório.', 'error'); return; }
  const data = await api({ action: 'cliente_salvar' }, 'POST', body);
  if (data.erro) { toast(data.erro, 'error'); return; }
  toast('Cliente salvo!');
...
async function excluirCliente(id) {
  ...
  const fd = new FormData(); fd.append('id', id);
  const data = await apiPost('cliente_excluir', fd);
  if (data.erro) { toast(data.erro, 'error'); return; }
  toast('Cliente removido!');
  loadClientes();
  loadEmpResumo(empState.empresaId);
}

// ── SERVIÇOS CRUD ─────────────────────────────
async function loadClientesNoSelect() {
  const empId = document.getElementById('servicoEmpresaId').value;
  const sel = document.getElementById('servicoClienteId');
  sel.innerHTML = '<option value="">Selecione o cliente</option>';
  if (!empId) return;
  const data = await api({ action: 'clientes_listar', empresa_id: empId });
  (data.clientes || []).forEach(c => { sel.innerHTML += `<option value="${c.id}">${esc(c.nome)}</option>`; });
}

function openServicoModal(isGeral = false) {
  document.getElementById('servicoId').value  = '';
  document.getElementById('servicoModalTitle').textContent = 'Novo Serviço de TI';
  ...
  document.getElementById('servicoVenc').value  = new Date().toISOString().split('T')[0];
  document.getElementById('servicoRecorrente').checked = false;

  if (!isGeral && empState.empresaId) {
    document.getElementById('servicoEmpresaId').value = empState.empresaId;
    loadClientesNoSelect();
  } else {
    document.getElementById('servicoEmpresaId').value = '';
    document.getElementById('servicoClienteId').innerHTML = '<option value="">Selecione o cliente</option>';
  }
  document.getElementById('modalServico').classList.add('open');
  document.getElementById('servicoNome').focus();
}

function novoServicoParaCliente(clienteId, clienteNome) {
  openServicoModal(false);
  setTimeout(() => {
    document.getElementById('servicoEmpresaId').value = empState.empresaId;
    loadClientesNoSelect().then(() => {
      document.getElementById('servicoClienteId').value = clienteId;
    });
  }, 50);
}

async function editarServico(id) {
  const data = await api({ action: 'servico_buscar', id });
  ...
  document.getElementById('servicoEmpresaId').value = s.empresa_id;
  await loadClientesNoSelect();
  document.getElementById('servicoClienteId').value = s.cliente_id;
  document.getElementById('servicoNome').value  = s.nome_servico;
  document.getElementById('servicoDesc').value  = s.descricao || '';
  document.getElementById('servicoValor').value = parseFloat(s.valor).toFixed(2).replace('.', ',');
  document.getElementById('servicoVenc').value  = s.data_vencimento;
  document.getElementById('servicoRecorrente').checked = s.recorrente == 1;
  document.getElementById('modalServico').classList.add('open');
}
...
async function salvarServico() {
  const body = {
    ...
    empresa_id:      document.getElementById('servicoEmpresaId').value,
    cliente_id:      document.getElementById('servicoClienteId').value,
    nome_servico:    document.getElementById('servicoNome').value.trim(),
    descricao:       document.getElementById('servicoDesc').value.trim(),
    valor:           document.getElementById('servicoValor').value,
    data_vencimento: document.getElementById('servicoVenc').value,
    recorrente:      document.getElementById('servicoRecorrente').checked ? 1 : 0,
  };
  if (!body.nome_servico || !body.empresa_id || !body.cliente_id || !body.data_vencimento) {
    toast('Preencha empresa, cliente, nome e vencimento.', 'error'); return;
  }
  const data = await api({ action: 'servico_salvar' }, 'POST', body);
  if (data.erro) { toast(data.erro, 'error'); return; }
  toast('Serviço salvo!');
  closeModal('modalServico');
  if (empState.tabAtual === 'servicos') loadServicosEmp();
  else loadClientes();
  loadEmpResumo(empState.empresaId || body.empresa_id);
  if (state.view === 'emp-servicos') loadServicosGeral();
}

async function loadServicosEmp() {
  if (!empState.empresaId) return;
  const data = await api({ action: 'servicos_listar', empresa_id: empState.empresaId });
  renderTabelaServicos(data.servicos || [], 'tabelaServicosEmp', false);
}

async function loadServicosGeral() {
  const empresa_id = document.getElementById('filtroEmpServico').value;
  const status     = document.getElementById('filtroStatusServico').value;
  const data = await api({ action: 'servicos_listar', empresa_id, status });
  const rows = data.servicos || [];
  document.getElementById('servicosGeralTitle').textContent = `${rows.length} serviço${rows.length!==1?'s':''} de TI`;
  renderTabelaServicos(rows, 'tabelaServicosGeral', true);
}

function renderTabelaServicos(rows, tbId, showEmpresa) {
  const tb = document.getElementById(tbId);
  if (!rows.length) { tb.innerHTML = `<tr><td colspan="${showEmpresa?8:7}"><div class="empty"><p>Nenhum serviço encontrado.</p></div></td></tr>`; return; }
  tb.innerHTML = rows.map(s => {
    const dias = parseInt(s.dias_para_vencer ?? 0);
    let vencChip;
    if (s.status === 'pago') {
      vencChip = `<span class="venc-chip ok">${fmtDate(s.data_vencimento)}</span>`;
    } else if (dias < 0) {
      vencChip = `<span class="venc-chip atrasado">⚠ ${Math.abs(dias)}d atrasado</span>`;
    } else if (dias === 0) {
      vencChip = `<span class="venc-chip hoje">🟡 Hoje</span>`;
    } else if (dias <= 5) {
      vencChip = `<span class="venc-chip proximo">🔵 ${dias}d</span>`;
    } else {
      vencChip = `<span class="venc-chip ok">${fmtDate(s.data_vencimento)}</span>`;
    }
    const statusCell = s.status === 'pago'
      ? `<span class="badge badge-green">✓ Pago<br><span style="font-weight:400;font-size:.65rem">${fmtDate(s.data_pagamento)}</span></span>`
      : `<span class="badge badge-yellow">⏳ Pendente</span>`;

    const payBtn = s.status === 'pendente'
      ? `<button class="btn btn-primary btn-sm" onclick="openPagarServico(${s.id},'${esc(s.nome_servico)}')" title="Marcar como recebido">✅</button>`
      : `<button class="btn btn-secondary btn-sm btn-icon" onclick="estornarServico(${s.id})" title="Estornar">↩️</button>`;

    return `<tr>
      <td style="font-weight:500">🖥️ ${esc(s.nome_servico)}${s.recorrente=='1'?' <span style="font-size:.65rem;color:var(--blue)">↺</span>':''}</td>
      ${showEmpresa ? `<td style="font-size:.8rem;color:var(--text2)">${esc(s.empresa_nome)}</td>` : ''}
      <td style="font-size:.8rem">${esc(s.cliente_nome)}</td>
      <td>${vencChip}</td>
      <td>${s.status==='pendente'&&dias<0?`<span class="venc-chip atrasado">Vencido</span>`:`<span class="venc-chip ok">${fmtDate(s.data_vencimento)}</span>`}</td>
      <td><span class="valor-mono chip-receita">${fmtMoney(s.valor)}</span></td>
      <td>${statusCell}</td>
      <td><div class="actions">
        ${payBtn}
        <button class="btn btn-secondary btn-sm btn-icon" onclick="editarServico(${s.id})" title="Editar">✏️</button>
        <button class="btn btn-danger btn-sm btn-icon" onclick="excluirServico(${s.id})" title="Excluir">🗑️</button>
      </div></td>
    </tr>`;
  }).join('');
}

function openPagarServico(id, desc) {
  document.getElementById('pagarServicoId').value = id;
  document.getElementById('pagarServicoDesc').textContent = desc;
  document.getElementById('dataPagamentoServico').value = new Date().toISOString().split('T')[0];
  document.getElementById('modalPagarServico').classList.add('open');
}

async function confirmarPagamentoServico() {
  const fd = new FormData();
  fd.append('id', document.getElementById('pagarServicoId').value);
  fd.append('data_pagamento', document.getElementById('dataPagamentoServico').value);
  const data = await apiPost('servico_pagar', fd);
  if (data.erro) { toast(data.erro, 'error'); return; }
  toast(data.mensagem || 'Recebido!');
  closeModal('modalPagarServico');
  if (empState.tabAtual === 'servicos') loadServicosEmp();
  if (state.view === 'emp-servicos') loadServicosGeral();
  if (empState.empresaId) loadEmpResumo(empState.empresaId);
}

async function estornarServico(id) {
  if (!confirm('Estornar este recebimento?')) return;
  const fd = new FormData(); fd.append('id', id);
  const data = await apiPost('servico_estornar', fd);
  if (data.erro) { toast(data.erro, 'error'); return; }
  toast('Estornado!');
  if (empState.tabAtual === 'servicos') loadServicosEmp();
  if (state.view === 'emp-servicos') loadServicosGeral();
...
async function excluirServico(id) {
  ...
  const fd = new FormData(); fd.append('id', id);
  const data = await apiPost('servico_excluir', fd);
  if (data.erro) { toast(data.erro, 'error'); return; }
...
function icon($name, $size = 16) {
    $icons = [
        ...
        'building'         => '<svg width="'.$size.'" height="'.$size.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M9 22V12h6v10M9 7h1M14 7h1M9 12h1M14 12h1"/></svg>',
